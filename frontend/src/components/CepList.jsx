...
export default function CepList({ searchQuery }) {
  ...
  const handleToggleFavorite = async (cepId, currentStatus) => {
    try {
      const response = await fetch(`http://localhost:3000/ceps/${cepId}/toggle-favorite`, {
        method: 'PATCH',
        headers: {
          'Content-Type': 'application/json',
        },
    ...
    <div className="space-y-4">
      {filteredCeps.map((cep) => (
        ...
        >
          ...
          <div className="flex flex-col items-center text-gray-400 ml-4 space-y-2">
            <MapPin size={20} />
            <span className="text-xs">{cep.ddd}</span>

            {/* <button onClick={() => handleToggleFavorite(cep.id, cep.favorite)}>
              {cep.favorite ? (
                <Star size={20} className="text-yellow-500" />
              ) : (
                <StarOff size={20} className="hover:text-yellow-400" />
              )}
            </button> */}
...
{cep.favoritado ? (
  <Star
    ...
    className="text-yellow-500 cursor-pointer hover:brightness-125 transition"
  ...
  <StarOff
    size={20}
    onClick={() => handleToggleFavorite(cep.id, cep.favoritado)}
    className="text-gray-400 cursor-pointer hover:text-yellow-400 transition"
...
//   const handleEdit = (cepId) => {
//     setEditMode((prev) => ({ ...prev, [cepId]: true }));
//     setOriginalData((prev) => ({ ...prev, [cepId]: ceps.find((cep) => cep.id === cepId) })); // Salvar dados originais
//   };
...
//         setCeps((prevCeps) =>
//           prevCeps.map((cep) =>
//             cep.id === cepId ? { ...cep, favorite: !currentStatus } : cep
//           )
//         );
//       }
//     } catch (error) {
